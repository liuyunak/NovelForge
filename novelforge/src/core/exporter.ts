...
export interface ExportOptions {
  ...
  /** Path to cover image file (PNG/JPEG). If provided, embeds into PDF/EPUB cover page. */
...
export class Exporter {
  ...
  /**
   * Export novel to a single format.
   */
  async export(options: ExportOptions): Promise<string> {
    const chapters = await this.loadChapters(options.chapterRange)
    const metadata = this.getMetadata()

    options.onProgress?.(0, chapters.length, 'loading')
    ...
    options.onProgress?.(0, chapters.length, `exporting_${options.format}`)
    ...
    options.onProgress?.(chapters.length, chapters.length, 'complete')

    // Record export in history
    this.recordExport(filename, options.format, chapters.length)
   ...
   */
  async batchExport(options: BatchExportOptions): Promise<{ format: ExportFormat; filename: string; url: string }[]> {
    const results: { format: ExportFormat; filename: string; url: string }[] = []
    const total = options.formats.length

    for (let i = 0; i < options.formats.length; i++) {
      const fmt = options.formats[i]
      options.onProgress?.(i, total, `exporting_${fmt}`)

      const outputPath = await this.export({
        format: fmt,
  ...
  /**
   * Get export history for this workspace.
   */
  getExportHistory(): ExportRecord[] {
    const historyPath = path.join(this.workspacePath, 'exports', '.export_history.json')
    try {
  ...
  deleteExportFile(filename: string): boolean {
    const filePath = path.join(this.workspacePath, 'exports', filename)
    // Safety: prevent path traversal
    const resolved = path.resolve(filePath)
    const exportsDir = path.resolve(path.join(this.workspacePath, 'exports'))
    ...
    if (fs.existsSync(filePath)) {
      ...
      // Update history
      this.removeExportRecord(filename)
      return true
    }
    return false
  }
  ...
  /**
   * Get list of exported files in the workspace.
   */
  listExportFiles(): { filename: string; size: number; created: Date }[] {
    const exportsDir = path.join(this.workspacePath, 'exports')
    if (!fs.existsSync(exportsDir)) return []

    return fs.readdirSync(exportsDir)
      .filter(f => !f.startsWith('.') && !f.startsWith('scripts'))
      .map(f => {
  ...
  /**
   * Record an export in the history file.
   */
  private recordExport(filename: string, format: ExportFormat, chapterCount: number): void {
    const historyPath = path.join(this.workspacePath, 'exports', '.export_history.json')
   ...
   */
  private removeExportRecord(filename: string): void {
    const historyPath = path.join(this.workspacePath, 'exports', '.export_history.json')
    try {
      if (fs.existsSync(historyPath)) {
        const history: ExportRecord[] = JSON.parse(fs.readFileSync(historyPath, 'utf-8'))
...
<w:styles xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
  ...
  private async saveAsPdf(chapters: ChapterData[], metadata: NovelMetadata, options: ExportOptions, outputPath: string): Promise<void> {
    const coverBase64 = options.coverImagePath ? this.loadCoverAsBase64(options.coverImagePath) : null
    const html = this.buildPdfHtml(chapters, metadata, coverBase64)
    fs.writeFileSync(outputPath, html, 'utf-8')
  }

  private buildPdfHtml(chapters: ChapterData[], metadata: NovelMetadata, coverBase64?: string | null): string {
    let chapterHtml = ''
    for (const chapter of chapters) {
    ...
    // Build cover section with optional embedded image
    let coverContent: string
    if (coverBase64) {
      coverContent = `
    <img class="cover-image" src="${coverBase64}" alt="封面" />
    <h1 class="cover-title">${this.escapeHtml(metadata.title)}</h1>
...
<body>
  ...
  private async saveAsEpub(chapters: ChapterData[], metadata: NovelMetadata, options: ExportOptions, outputPath: string): Promise<void> {
    const coverBase64 = options.coverImagePath ? this.loadCoverAsBase64(options.coverImagePath) : null
    const epubFiles = this.buildEpubFiles(chapters, metadata, coverBase64)
    await this.createZip(epubFiles, outputPath)
  }

  private buildEpubFiles(chapters: ChapterData[], metadata: NovelMetadata, coverBase64?: string | null): Map<string, string> {
    const files = new Map<string, string>()
...
<container version="1.0" xmlns="urn:oasis:names:tc:opendocument:xmlns:container">
  <rootfiles>
    ...
    // Cover page
    let coverBodyContent: string
    if (coverBase64) {
      // If cover image is available, embed it in the cover page
      coverBodyContent = `<div style="text-align:center; padding-top:10%;">
    <img src="${coverBase64}" alt="封面" style="max-width:60%; max-height:50vh; margin-bottom:2em; border-radius:4px;"/>
    ...
    ${metadata.genre ? `<p>题材：${this.escapeXml(metadata.genre)}</p>` : ''}
      ...
      coverBodyContent = `<div style="text-align:center; padding-top:30%;">
    <h1>${this.escapeXml(metadata.title)}</h1>
    ${metadata.author ? `<p>作者：${this.escapeXml(metadata.author)}</p>` : ''}
    ${metadata.genre ? `<p>题材：${this.escapeXml(metadata.genre)}</p>` : ''}
  </div>`
...
<package xmlns="http://www.idpf.org/2007/opf" version="3.0" unique-identifier="book-id">
  ...
  /**
   * Load a cover image file and return it as a base64 data URI.
   * Returns null if the file doesn't exist or can't be read.
   */
  private loadCoverAsBase64(imagePath: string): string | null {
    try {
      // Path traversal prevention: only allow images within the workspace directory
      const resolved = path.resolve(imagePath)
      const workspaceDir = path.resolve(this.workspacePath)
      if (!resolved.startsWith(workspaceDir)) {
        logger.warn(`[Exporter] Cover image path traversal blocked: ${imagePath}`)
        return null
      }
      ...
      if (!allowedExts.includes(ext)) {
        logger.warn(`[Exporter] Cover image rejected — unsupported extension: ${ext}`)
        return null
      }
      
      // Maximum cover image size: 10MB to prevent OOM
      const MAX_COVER_SIZE_BYTES = 10 * 1024 * 1024
      const stat = fs.statSync(resolved)
      if (stat.size > MAX_COVER_SIZE_BYTES) {
        logger.warn(`[Exporter] Cover image too large: ${imagePath} (${(stat.size / 1024 / 1024).toFixed(1)}MB, max 10MB)`)
        return null
      }
      if (stat.size === 0) {
        logger.warn(`[Exporter] Cover image is empty: ${imagePath}`)
        return null
      }
      ...
      logger.debug(`[Exporter] Cover image loaded: ${imagePath} (${(imageData.length / 1024).toFixed(1)}KB)`)
      return `data:${mimeType};base64,${base64}`
    } catch (err) {
      logger.warn(`[Exporter] Failed to load cover image: ${imagePath} %s`, err)
      return null
    }
  }
